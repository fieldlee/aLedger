...
func LedgerIssue(stub shim.ChaincodeStubInterface)pb.Response{
	...
	issueParam.Holder = common.Trim(common.ADMIN_Name)
	...
	if issueByte != nil {
		...
		leder.Amount = common.FloatAdd(leder.Amount , issueParam.Amount)
		leder.Desc = fmt.Sprintf("%s issue %s token amount:%s",common.ADMIN_Name,token.Name,strconv.FormatFloat(issueParam.Amount,'f',2,64))
	}else{
		leder.Holder = holder
	...
	///////////////================修改token
	if token.Amount > 0 {
		token.Amount = common.FloatAdd( token.Amount , issueParam.Amount)
	}else{
...
func LedgerBurnBalance(stub shim.ChaincodeStubInterface)pb.Response{
	...
	//////////// holder
	key, err := stub.CreateCompositeKey(common.CompositeIndexName, []string{common.Ledger_PRE, strings.ToUpper(token.Name),  strings.ToUpper(common.ADMIN_Name)})
	if err != nil {
		return common.SendError(common.COMPOSTEKEY_ERR,fmt.Sprintf("Could not create a composite key for %s-%s: %s", token.Name, common.ADMIN_Name, err.Error()))
	}
	...
	ledger.Amount = common.FloatSub(ledger.Amount , burnParam.Amount)
...
func LedgerTransfer(stub shim.ChaincodeStubInterface)pb.Response{
	...
	address , err := key2.SignToAddress(args[0],args[1])
	if err != nil {
		return common.SendError(common.MARSH_ERR,err.Error())
	}
	...
	///// check signature private key
	if common.Trim(accountFrom.Address) != common.Trim(address) {
		return common.SendError(common.ACCOUNT_COMMONNAME,"sign account error,please check your private key")
	}
	...
	ledger.Amount = common.FloatSub( ledger.Amount , transfer.Amount )
	...
	if toledgerByte == nil {
		...
		toledger.Amount = common.FloatAdd( toledger.Amount , transfer.Amount )
	}else {
