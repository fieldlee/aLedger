...
func SignRequest(stub shim.ChaincodeStubInterface)pb.Response{
	...
	//// account receiver
	account,err := AccountGetByName(stub,request.Receiver)
	if err != nil {
		return common.SendError(common.ACCOUNT_COMMONNAME,err.Error())
	}

	///// check signature private key
	if common.Trim(account.Address) != common.Trim(address) {
	...
	txid := stub.GetTxID()
	from := common.Trim(request.Sender)
...
func SignHistory(stub shim.ChaincodeStubInterface)pb.Response {
	...
	key, err := stub.CreateCompositeKey(common.CompositeRequestIndexName, []string{common.SIGN_PRE,token.Name,common.Trim(response.Sender),response.Txid})
	if err != nil {
...
func SignRepsonse(stub shim.ChaincodeStubInterface)pb.Response  {
	...
	accountSender,err := AccountGetByName(stub,response.Sender)
	if err != nil {
		return common.SendError(common.MARSH_ERR,err.Error())
	}

	///// check signature private key
	if common.Trim(accountSender.Address) != common.Trim(address) {
		return common.SendError(common.ACCOUNT_COMMONNAME,"sign account error,please check your private key")
	}
	...
	if response.Accept == true{  ///// 同意支付
		...
		ledger.Amount = common.FloatSub( ledger.Amount , sign.Amount )
		...
		if toledgerByte == nil {
			...
			toledger.Amount = common.FloatAdd( toledger.Amount , sign.Amount )
		}else {
